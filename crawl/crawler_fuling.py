#!/usr/bin/env python3
# 富凌 chinafuling.com 变频器手册采集器
# 下载页 https://www.chinafuling.com/download-N.html
import json, os, re, time, subprocess

UA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
BASE = 'https://www.chinafuling.com'
OUT = 'pdfs/fuling'
MANIFEST = 'fuling_manifest.json'

def fetch(url, timeout=60):
    r = subprocess.run(['curl', '-s', '-L', '--max-time', str(timeout),
        '-A', UA, '-e', f'{BASE}/download-3.html', url],
    ...
    for url, name in all_items.items():
        ...
        try:
            ...
            if pages == 0:
                ...
                continue
            ftype = '使用手册'
            if '样本' in name or '画册' in name or 'catalog' in name.lower():
                ftype = '产品样本'
            elif '简易' in name:
                ftype = '简易手册'
            manifest.append({'name': name, 'url': url, 'pages': pages,
                           'type': ftype, 'size': len(data),
                           'brand': '富凌', 'src': 'chinafuling.com'})
            seen.add(url)
