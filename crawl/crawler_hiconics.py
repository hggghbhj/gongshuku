#!/usr/bin/env python3
"""合康新能 HICONICS 下载中心采集器
PDF直链: https://www.hiconics.com/upload/file/日期/uuid.pdf
"""
import json, os, time, urllib.request, urllib.parse, subprocess, re, sys

BASE = os.path.dirname(os.path.abspath(__file__))
PDF_DIR = os.path.join(BASE, '..', 'pdfs', 'hiconics')
MANIFEST = os.path.join(BASE, 'hiconics_manifest.json')
DOWNLOAD_PAGE = 'https://www.hiconics.com/category/download1.html'
HEADERS = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
           'Referer': 'https://www.hiconics.com/'}
...
def run():
    ...
    try:
        ...
        print(f'抓取页面失败: {e}')
        return {'brand': 'hiconics', 'new': 0, 'fail': 0, 'total': len(old)}
    ...
    for url in pdfs:
        if url.startswith('//'):
            ...
            url = 'https://www.hiconics.com' + url
        elif not url.startswith('http'):
            url = 'https://www.hiconics.com/' + url
        if 'hiconics' not in url: continue
        # 从URL提取日期作为名称参考
        name = url.split('/')[-1].replace('.pdf','')
        if re.match(r'^[a-f0-9-]{36}$', name):
            name = '合康说明书_' + name[:8]
    ...
    print(f'合康新能完成: 新增{ok}, 失败{fail}, 总计{len(manifest)}')
    return {'brand': 'hiconics', 'new': ok, 'fail': fail, 'total': len(manifest)}
