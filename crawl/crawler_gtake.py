#!/usr/bin/env python3
"""吉泰科 GTAKE 下载中心采集器
WordPress站点，PDF直链: https://www.gtake.com/wp-content/uploads/...
"""
import json, os, time, urllib.request, urllib.parse, subprocess, re, sys

BASE = os.path.dirname(os.path.abspath(__file__))
PDF_DIR = os.path.join(BASE, '..', 'pdfs', 'gtake')
MANIFEST = os.path.join(BASE, 'gtake_manifest.json')
DOWNLOAD_PAGE = 'https://www.gtake.com/downloads/'
HEADERS = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
           'Referer': 'https://www.gtake.com/'}
...
def run():
    ...
    try:
        ...
        print(f'抓取页面失败: {e}')
        return {'brand': 'gtake', 'new': 0, 'fail': 0, 'total': len(old)}
    ...
    for url in pdfs:
        if url.startswith('//'):
            ...
            url = 'https://www.gtake.com' + url
        elif not url.startswith('http'):
            url = 'https://www.gtake.com/' + url
        if 'gtake' not in url: continue
    ...
    print(f'吉泰科完成: 新增{ok}, 失败{fail}, 总计{len(manifest)}')
    return {'brand': 'gtake', 'new': ok, 'fail': fail, 'total': len(manifest)}
